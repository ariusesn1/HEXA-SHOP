...
/**
 * Servlet implementation class LoadDataAcc
 */
@WebServlet("/LoadDataAcc")
public class LoadDataAcc extends HttpServlet {
...
//		session.setAttribute("listDM", danhSachDanhMuc);
		...
		//Lấy 1 danh mục id = 2
		Account acc = AccountDao.findById(2);
		javax.servlet.http.HttpSession session = request.getSession();
		session.setAttribute("acc2", acc);
