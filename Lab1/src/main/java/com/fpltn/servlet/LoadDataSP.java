...
/**
 * Servlet implementation class LoadDataSP
 */
@WebServlet("/LoadDataSP")
public class LoadDataSP extends HttpServlet {
...
//		session.setAttribute("listDM", danhSachDanhMuc);
		...
		//Lấy 1 danh mục id = 2
		Sanpham sp = SanPhamDao.findById(2);
		javax.servlet.http.HttpSession session = request.getSession();
		session.setAttribute("sp2", sp);
